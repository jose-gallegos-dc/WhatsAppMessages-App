...
<template>
    <v-row>
        <v-col cols="4" class="pr-1">
            <v-select class="text-teal-darken-2" v-model="countryCodeSelected" :items="countryCodeItems" item-value="code"
                label="País" return-object :item-title="(item) => `${item.country}  (${item.code})`">
                <template v-slot:selection="{ item }">
                    <v-icon v-if="item.value">{{ item.value.icon }}</v-icon>
                </template>
        ...
        <v-col cols="8" class="pl-1">
            <v-autocomplete class="text-teal-darken-2"
                label="Número de teléfono" v-on:keypress="NumbersOnly" v-model.lazy="phoneNumberSelected"
                v-model:search="phoneNumberSearch" :items="phoneNumberItems" :loading="phoneNumberLoading" hide-no-data
                clearable @change="onPhoneNumberChange" :hint="isOriginalPhoneNumber" persistent-hint/>
        </v-col>
    </v-row>
</template>
